...
program
  ...
  .action((name, options) => {
    ...
    if (options.path) {
      ...
      trimmedPath = componentFolder.substring(componentFolder.indexOf('src'))
      ...
      trimmedPath = `src/components/${name}`
      reexportStatement = `export { default as ${name} } from 'src/components/${name}'\n`
    }
